...
export default function AgendaCalendar({ isAdmin = false }: Props) {
  ...
  // Agrupa eventos por perfil
  const eventsByPerfil: Record<Perfil, AgendaEvent[]> = {
    Confi: [],
    Cecília: [],
    Luiza: [],
    Júlio: [],
  ...
  events.forEach(ev => {
    if (ev.perfil && eventsByPerfil[ev.perfil]) eventsByPerfil[ev.perfil].push(ev);
  });
    ...
    <div style={{ display: 'flex', gap: 24 }}>
          ...
          <div style={{ marginTop: 8, textAlign: 'left', border: '1px solid #ccc', padding: 8, borderRadius: 4 }}>
            ...
            {profiles.map(p => (
              <div key={p} style={{ marginTop: 8 }}>
                <strong>{p}:</strong> {perfilMap[p]?.chatId}
                {isUserAdmin && (
                  <div>
                    <input
      ...
      <div style={{ flex: 3 }}>
        ...
        <FullCalendar
          ...
          selectable
          editable
          events={filterProfile === 'Todos'
            ? events.map(ev => ({ ...ev, color: ev.perfil ? perfilColors[ev.perfil] : '#999' }))
            : eventsByPerfil[filterProfile].map(ev => ({ ...ev, color: perfilColors[filterProfile] }))
          }
