  ...
  useEffect(() => {
    async function fetchChecklist() {
      try {
        ...
        // Remove duplicados pelo ID e pega apenas tarefas do dia
        const todayTasks = Array.from(new Map(data.map(t => [t.id, t])).values())
          .filter(t => t.date.slice(0, 10) === today && !t.done);
  ...
  const saveEvent = async (ev: AgendaEvent, isEdit = false) => {
    try {
      if (isEdit) {
        ...
        // Se o evento tiver tarefa, adiciona no checklist instantaneamente
        if (ev.tarefa) {
          ...
          if (newTask.date.slice(0, 10) === today) {
