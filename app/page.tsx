  ...
  return (
    <div className="flex h-screen bg-white font-sans overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto space-y-4 no-scrollbar custom-scrollbar">
              {feed.map((item, idx) => (
                <div key={idx} className="bg-white border-2 border-black p-4 rounded-2xl shadow-[4px_4px_0px_black]">
                  ...
                  <p className={`text-[10px] font-black mt-1 ${item.Tipo === 'RESPOSTA' ? 'text-[#1260c7]' : 'opacity-70'}`}>
      ...
      <main className="flex-1 overflow-y-auto p-8 no-scrollbar bg-white relative">
        ...
        <div className="relative mb-10 h-72 rounded-[40px] border-2 border-black overflow-hidden flex items-end p-8 z-0">
          ...
          <div className="relative z-50 flex items-center justify-between w-full">
            <div className="flex items-center gap-6">
              ...
              <div className="relative">
                ...
                {showPerfilSelector && (
                    <div className="absolute top-full left-0 mt-2 w-full bg-white border-2 border-black rounded-2xl shadow-xl z-[150] overflow-hidden">
                        {perfis.map(p => (
          ...
          {Array.from({ length: diasNoMes }, (_, i) => {
            ...
            return (
              <div key={dia} onClick={() => { setTempEvento({...tempEvento, id:'', dataInicio:key, dataTermino:key, perfil:perfilAtivo.nome, chatId:perfilAtivo.chatId, titulo: '', conteudoSecundario: ''}); setShowEventModal(true); }} className={`h-32 border-2 border-black rounded-[30px] p-4 cursor-pointer transition-all hover:scale-105 ${dataAtiva.getDate() === dia && dataAtiva.getMonth() === new Date().getMonth() ? 'bg-orange-50' : 'bg-white'}`}>
                ...
                <div className="flex gap-1 mt-2 flex-wrap">{evs.map((ev, idx) => <div key={idx} onClick={(e) => { e.stopPropagation(); setTempEvento({...ev, dataInicio:ev.dataInicio.split(' ')[0], dataTermino:ev.dataFim.split(' ')[0], horaInicio:ev.dataInicio.split(' ')[1] || '08:00', horaFim:ev.dataFim.split(' ')[1] || '09:00'}); setShowEventModal(true); }} className="w-3 h-3 rounded-full border border-black shadow-sm" style={{ backgroundColor: ev.cor }} />)}</div>
              </div>
            );
          })}
        </div>
            ...
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white border-2 border-black rounded-[60px] p-12 w-full max-w-3xl shadow-2xl max-h-[90vh] overflow-y-auto no-scrollbar">
              ...
              <div className="space-y-8">
                ...
                <div className="flex justify-between items-center pt-8 border-t-2 border-black font-black uppercase text-2xl">
                  <div className="flex gap-8"><button onClick={handleSalvar} className="hover:underline decoration-[#ffce0a] decoration-[12px]">Gravar</button><button onClick={handleDispararWhatsApp} disabled={enviandoZap} className={`flex items-center gap-2 ${enviandoZap ? 'opacity-20' : 'hover:underline text-[#1260c7]'}`}><Send size={24} /> {enviandoZap ? '...' : 'Disparar'}</button></div>
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/60 backdrop-blur-md p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white border-2 border-black rounded-[50px] p-12 w-full max-w-xl shadow-2xl relative">
              ...
              <div className="space-y-6">
                ...
                <div><p className="text-[10px] font-black uppercase opacity-30 mb-2">WhatsApp ChatID</p><input value={perfilAtivo?.chatId || ''} onChange={e => setPerfilAtivo({...perfilAtivo, chatId: e.target.value})} className="w-full border-2 border-black rounded-2xl p-4 font-bold" /></div>
