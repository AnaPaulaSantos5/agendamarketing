...
import { Settings, ChevronDown, ChevronLeft, ChevronRight, X, Camera, Plus, Clock, Calendar as CalendarIcon, Trash2 } from 'lucide-react';
...
export default function AgendaPage() {
  ...
  const handleDiaClick = (dia: number) => {
    ...
    // Verifica se já existe evento para carregar no modal (Edição)
    const existente = eventos.find(e => e.dataInicio?.startsWith(dataStr));
    ...
    if (existente) {
      setTempEvento({
        ...
        dataInicio: dataStr,
        dataTermino: existente.dataFim?.split(' ')[0] || dataStr,
        cor: existente.cor,
  ...
  const handleSalvar = async () => {
    ...
    if (res.ok) {
      ...
      carregarDados(); // Recarrega sem dar refresh na página toda
    }
  };
  ...
  return (
    <div className="p-5 max-w-[1400px] mx-auto bg-white min-h-screen font-sans no-scrollbar">
      ...
      <div className="relative mb-10 h-80 rounded-[40px] border-[3px] border-black overflow-hidden flex items-end p-8 shadow-[10px_10px_0px_black]">
        ...
        <div onClick={() => fileInputRef.current?.click()} className="absolute inset-0 cursor-pointer z-0 bg-gray-50 flex items-center justify-center italic text-gray-400 font-bold">
        ...
        <div className="relative z-10 flex items-center gap-6">
            ...
            <AnimatePresence>
              {showPerfilSelector && (
                <motion.div initial={{opacity:0, y:10}} animate={{opacity:1, y:0}} className="absolute top-28 left-0 bg-white border-2 border-black rounded-2xl p-4 shadow-xl w-48 z-50">
                  <p className="text-[10px] font-black uppercase opacity-30 mb-2">Trocar Perfil</p>
                  {perfis.map(p => (
          ...
          <div className="bg-white border-[3px] border-black px-6 py-2 rounded-2xl shadow-[5px_5px_0px_black] flex items-center gap-4">
            <div>
               <h3 className="text-2xl font-black italic uppercase leading-none">{perfilAtivo?.nome || "Carregando..."}</h3>
      ...
      <div className="grid grid-cols-7 gap-4">
        ...
        {Array.from({ length: diasNoMes }, (_, i) => {
          ...
          const eventosDoDia = eventos.filter(e => e.dataInicio?.startsWith(key));
          ...
          return (
            <div key={dia} onClick={() => handleDiaClick(dia)} className={`h-36 border-2 border-black rounded-[30px] p-4 cursor-pointer transition-all hover:scale-105 flex flex-col justify-between ${dataAtiva.getDate() === dia ? 'bg-orange-50 border-orange-500 shadow-lg' : 'bg-white'}`}>
              <span className="text-2xl font-black italic tracking-tighter">{dia}</span>
            ...
            <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white border-[4px] border-black rounded-[60px] p-12 w-full max-w-3xl shadow-[30px_30px_0px_rgba(0,0,0,1)] max-h-[90vh] overflow-y-auto no-scrollbar">
              ...
              <div className="flex justify-between items-center mb-10 border-b-4 border-black pb-4">
                ...
                <div className="flex gap-2 items-center">
                  <p className="text-[10px] font-black uppercase opacity-20 mr-2">Cores</p>
                  {CORES_PASTEL.slice(0, 6).map(c => (
                  ...
                  <button className="w-7 h-7 rounded-full border-2 border-black border-dashed flex items-center justify-center hover:bg-gray-100"><Plus size={14}/></button>
                </div>
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="border-b-4 border-black/10 hover:border-black transition-colors">
                    ...
                    <input value={tempEvento.titulo} onChange={e => setTempEvento({...tempEvento, titulo: e.target.value})} className="w-full text-4xl font-black italic bg-transparent outline-none uppercase placeholder:opacity-5 py-2" placeholder="O QUE VAMOS POSTAR?" />
                  </div>
                  <div className="border-b-4 border-black/10 hover:border-black transition-colors">
                ...
                <div className="flex justify-between items-center pt-8 border-t-4 border-black font-black italic text-2xl uppercase tracking-tighter">
                  <div className="flex gap-10">
                    ...
                    {tempEvento.id && <button className="text-red-500 flex items-center gap-2 hover:scale-105 transition-transform"><Trash2 size={24}/> Excluir</button>}
                  </div>
