  ...
  return (
    <div className="flex h-screen bg-white font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-8 no-scrollbar bg-white relative">
        {/* HEADER AREA: Força Z-Index alto para ficar acima da imagem de capa */}
        <div className="relative z-30 flex items-center justify-between mb-6 h-20">
            <div className="flex items-center gap-6">
                <div className="relative z-40">
                    <div className="w-20 h-20 rounded-full border-2 border-black bg-white flex items-center justify-center text-4xl font-black shadow-[4px_4px_0px_black] uppercase overflow-hidden">{perfilAtivo?.nome?.charAt(0) || 'A'}</div>
            ...
            <button onClick={() => signOut()} className="bg-white border-2 border-black p-4 rounded-2xl font-black uppercase text-xs shadow-[4px_4px_0px_black] hover:bg-red-500 hover:text-white z-40">Sair</button>
        </div>

        {/* CAPA: Z-Index baixo para não bloquear cliques no cabeçalho */}
          ...
          <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white border-2 border-black rounded-[40px] p-10 w-full max-w-md shadow-[12px_12px_0px_black]">
              ...
              <div className="space-y-6 font-black uppercase text-xs">
                ...
                <div><p className="mb-1">ChatID WhatsApp</p><input value={perfilAtivo?.chatId || ''} onChange={e => setPerfilAtivo({...perfilAtivo, chatId: e.target.value})} className="w-full border-2 border-black p-4 rounded-xl outline-none" /></div>
