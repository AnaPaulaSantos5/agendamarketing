...
export default function AgendaPage() {
  ...
  const handleSalvarPerfilSheet = async () => {
    const res = await fetch('/api/agenda', { 
        method: 'POST', 
        body: JSON.stringify({ isPerfilUpdate: true, email: perfilAtivo.email, nome: perfilAtivo.nome, chatId: perfilAtivo.chatId }) 
    });
    if (res.ok) { alert("Sincronizado!"); setShowPerfilModal(false); carregarDados(); }
  };

  const handleExcluir = async () => {
    if (!tempEvento.id || !confirm("Excluir?")) return;
  ...
  const handleDiaClick = (dia: number) => {
    ...
    setTempEvento({ ...tempEvento, id: '', dataInicio: dataStr, dataTermino: dataStr, titulo: '', perfil: perfilAtivo?.nome || '', chatId: perfilAtivo?.chatId || '', conteudoSecundario: '', linkDrive: '' });
    const evs = eventos.filter(e => isDiaNoPeriodo(dataStr, e.dataInicio, e.dataFim));
  ...
  const handleEventoClick = (e: React.MouseEvent, evento: any) => {
    ...
    setTempEvento({ 
        ...
        cor: evento.cor, tipo: evento.tipo || 'externo', linkDrive: evento.linkDrive || '', 
    ...
    <div className="flex h-screen bg-white font-sans overflow-hidden">
      ...
      <aside className="w-80 border-r-2 border-black p-6 bg-gray-50 flex flex-col h-full overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto space-y-4 no-scrollbar custom-scrollbar">
          {feed.map((item, idx) => (
            <motion.div initial={{ x: -20 }} animate={{ x: 0 }} key={idx} className="bg-white border-2 border-black p-4 rounded-2xl shadow-[4px_4px_0px_black]">
              <div className="flex justify-between items-center mb-1">
                <span className={`text-[8px] font-black px-2 py-0.5 rounded-full border border-black uppercase text-white ${item.Tipo === 'ENVIO' ? 'bg-[#1260c7]' : 'bg-[#22c55e]'}`}>{item.Tipo}</span>
              ...
              {item.Tipo === 'RESPOSTA' ? (
                  <p className={`text-[10px] font-black mt-1 ${item.Resposta === 'SIM' ? 'text-[#1260c7] italic' : 'text-[#22c55e]'}`}>
                      {item.Resposta === 'SIM' ? 'Precisa de ajuda' : 'Está de acordo'}
                  </p>
              ) : <p className="text-[10px] opacity-70 mt-1 font-bold truncate italic">🚀 {item.Evento}</p>}
            </motion.div>
          ))}
        </div>
      </aside>
      ...
      <main className="flex-1 overflow-y-auto p-8 no-scrollbar bg-white relative">
        {/* CAPA E HEADER */}
        <div className="relative mb-10 h-64 rounded-[40px] border-2 border-black overflow-hidden flex items-end p-8 bg-gray-100">
          <div className="relative z-10 flex items-center justify-between w-full">
            <div className="flex items-center gap-6">
              ...
              <div className="bg-white border-2 border-black px-6 py-2 rounded-2xl flex items-center gap-4 cursor-pointer shadow-md" onClick={() => setShowPerfilSelector(!showPerfilSelector)}>
                <div><h3 className="text-xl font-black uppercase leading-none">{perfilAtivo?.nome || "Perfil"}</h3><p className="text-[9px] font-bold opacity-40 uppercase">{session?.user?.email}</p></div>
                <ChevronDown className="opacity-40" />
              </div>
            </div>
            <button onClick={() => setShowPerfilModal(true)} className="bg-white border-2 border-black p-4 rounded-2xl hover:bg-yellow-100 transition-all shadow-md"><Settings size={24} /></button>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center mb-8 border-b-2 border-black pb-4">
          <div className="flex items-center gap-4">
            ...
            <h1 className="text-8xl font-light tracking-tighter ml-4">{dataAtiva.getFullYear()}</h1>
        ...
        <div className="grid grid-cols-7 gap-4">
          ...
          {Array.from({ length: diasNoMes }, (_, i) => {
            ...
            return (
              <div key={dia} onClick={() => handleDiaClick(dia)} className={`h-32 border-2 border-black rounded-[30px] p-4 cursor-pointer transition-all hover:scale-105 ${dataAtiva.getDate() === dia ? 'bg-orange-50' : 'bg-white'}`}>
                <span className="text-2xl font-black">{dia}</span>
                <div className="flex gap-1 mt-2 flex-wrap">
                  {evs.map((ev, idx) => <div key={idx} onClick={(e) => handleEventoClick(e, ev)} className="w-3 h-3 rounded-full border border-black" style={{ backgroundColor: ev.cor }} />)}
                </div>
              </div>
            );
          })}
      ...
      {/* MODAL EVENTO COMPLETO */}
      <AnimatePresence>
        {showEventModal && (
          <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
            <motion.div initial={{ y: 50 }} animate={{ y: 0 }} className="bg-white border-2 border-black rounded-[60px] p-12 w-full max-w-3xl shadow-2xl overflow-y-auto max-h-[90vh] no-scrollbar">
              <div className="flex justify-between items-center mb-6 border-b-2 border-black pb-4">
                <h2 className="text-4xl font-black uppercase leading-none">Evento</h2>
                <div className="flex gap-2">{CORES_PASTEL.map(c => <div key={c} onClick={() => setTempEvento({...tempEvento, cor: c})} className={`w-6 h-6 rounded-full border-2 border-black cursor-pointer ${tempEvento.cor === c ? 'scale-125' : 'opacity-20'}`} style={{ backgroundColor: c }} />)}</div>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-100 p-4 rounded-xl border-2 border-black/5"><p className="text-[10px] font-black uppercase opacity-40">ChatId</p><p className="font-mono text-xs">{tempEvento.chatId}</p></div>
                    <div className="bg-gray-100 p-4 rounded-xl border-2 border-black/5"><p className="text-[10px] font-black uppercase opacity-40">Drive</p><input value={tempEvento.linkDrive} onChange={e => setTempEvento({...tempEvento, linkDrive: e.target.value})} className="w-full bg-transparent font-bold text-xs outline-none" placeholder="Link..." /></div>
                </div>
                <div className="flex justify-between items-center pt-8 border-t-2 border-black font-black uppercase text-2xl">
                  <div className="flex gap-8">
                    <button onClick={handleSalvar} className="hover:underline decoration-[#ffce0a] decoration-[10px]">Gravar</button>
        ...
        {showPerfilModal && (
          <div className="fixed inset-0 z-[500] flex items-center justify-center bg-black/40 backdrop-blur-md p-4">
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="bg-white border-2 border-black rounded-[50px] p-12 w-full max-w-xl shadow-2xl relative">
              <h2 className="text-4xl font-black uppercase mb-8 border-b-2 border-black pb-4">Perfil</h2>
              <div className="space-y-6">
                <div><p className="text-[10px] font-black uppercase opacity-30 mb-2">Nome</p><input value={perfilAtivo?.nome || ''} onChange={e => setPerfilAtivo({...perfilAtivo, nome: e.target.value})} className="w-full border-2 border-black rounded-2xl p-4 font-bold uppercase" /></div>
                <div><p className="text-[10px] font-black uppercase opacity-30 mb-2">ChatID</p><input value={perfilAtivo?.chatId || ''} onChange={e => setPerfilAtivo({...perfilAtivo, chatId: e.target.value})} className="w-full border-2 border-black p-4 rounded-xl font-bold" /></div>
                <button onClick={handleSalvarPerfilSheet} className="w-full bg-black text-white p-5 rounded-3xl font-black uppercase shadow-[6px_6px_0px_#ffce0a]">Gravar Planilha</button>
                <button onClick={() => setShowPerfilModal(false)} className="w-full font-black opacity-30 uppercase text-xs">Fechar</button>
              </div>
            </motion.div>
          </div>
        )}
